...
function extractDefaultVariant(variantUxRaw) {
  ...
  if (v.variants && typeof v.variants === "object") {
    ...
    const chosen =
      v.variants[defaultKey] ||
      v.variants.default ||
      v.variants[Object.keys(v.variants)[0]] ||
      null;
...
function normalizeCategories(f) {
  // supports category (single string) or categories (array of strings)
  const catArr = Array.isArray(f.categories) ? f.categories : null;
  const catSingle = typeof f.category === "string" ? [f.category] : null;

  const combined = [...(catArr || []), ...(catSingle || [])]
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Catalog error", details: msg },
      { status: 500 }
    );
  }
}
